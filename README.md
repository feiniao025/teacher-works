...
### AI 试卷批改（多模态）
- 上传试卷照片，多模态大模型自动逐题识别、判分并生成点评
- 支持配置**多个** OpenAI 兼容供应商（DeepSeek 官方 / 本地 LM Studio、Ollama、vLLM 等），一键切换，默认预设 DeepSeek
- 可全局开启 / 关闭 AI 功能；批改为**异步任务**，前端自动轮询进度
- 批改结果可人工复核后一键采纳写入成绩，并支持导出 Excel
- 详细配置见下文 [AI 试卷批改配置](#ai-试卷批改配置)
...
### AI 试卷批改
- `GET /api/v1/ai-grading/config` - 获取 AI 配置（开关 / 供应商列表 / 当前供应商）
- `PUT /api/v1/ai-grading/config` - 更新 AI 全局开关
- `GET /api/v1/ai-grading/presets` - 获取供应商快捷预设
- `POST /api/v1/ai-grading/providers` - 添加供应商
- `PUT /api/v1/ai-grading/providers/:id` - 编辑供应商
- `DELETE /api/v1/ai-grading/providers/:id` - 删除供应商
- `POST /api/v1/ai-grading/providers/:id/activate` - 切换当前使用的供应商
- `POST /api/v1/ai-grading/providers/:id/test` - 测试指定供应商连接（真实拉取模型）
- `POST /api/v1/ai-grading/test` - 测试当前启用的配置
- `GET /api/v1/ai-grading/tasks` - 批改任务列表
- `POST /api/v1/ai-grading/tasks` - 上传试卷图片创建批改任务（异步）
- `GET /api/v1/ai-grading/tasks/:id` - 查询任务状态 / 结果（前端轮询）
- `POST /api/v1/ai-grading/tasks/:id/adopt` - 采纳批改结果写入成绩
- `DELETE /api/v1/ai-grading/tasks/:id` - 删除任务
- `GET /api/v1/ai-grading/tasks/:id/export` - 导出批改结果 Excel
...
AI 试卷批改基于「多模态大模型 + OpenAI 兼容接口」实现：老师上传试卷照片，模型逐题识别、判分并生成点评，结果可人工复核后一键采纳写入成绩。该功能**默认关闭**，需先配置供应商并开启，且**测试用的本地模型不会写入代码**（仅保存在你自己的数据库中）。

### 一、开启入口
- 进入「AI 批改」页面，首次使用先点击「配置供应商」。
- 全局开关：可随时开启 / 关闭 AI 功能；关闭后不再发起任何模型调用，历史批改任务仍保留。

### 二、供应商配置（可配置多个、一键切换）
系统支持同时保存多个供应商并随时切换当前使用项；默认预设 DeepSeek，也可添加本地 / 内网模型服务。每个供应商包含：

| 配置项 | 说明 |
| --- | --- |
| 快捷预设 | 选择后自动带出常见 base_url / 模型名，可再手动修改 |
| 供应商名称 | 便于识别的自定义名称 |
| 服务地址 base_url | OpenAI 兼容地址，系统自动拼接 `/chat/completions`。**本地服务（LM Studio / Ollama / vLLM）通常需以 `/v1` 结尾**，如 `http://127.0.0.1:1234/v1`；若遗漏 `/v1`，系统会自动兜底重试 `/v1/chat/completions`，一般无需手动纠正 |
| API Key | 云端服务必填；本地模型通常可留空 |
| 模型名称 | 需为**支持图片输入的多模态模型**，如 `deepseek-v4-flash-vision-exp`、`qwen-vl-max` 等 |
| 多模态 | 批改需读取图片，请保持开启 |
| 温度 | 越低越稳定，批改建议 `0~0.3` |
| 流式响应 | **推荐开启**（默认开启）。开启后批改过程实时显示「模型思考中/作答中，已生成 N 字」，并以「空闲超时」替代「总时长超时」——只要模型持续输出就不会被误判超时，慢速推理模型批改长卷也能正常完成；关闭则回退为一次性返回 |
| 限制 Tokens | **推荐关闭**（默认关闭 = 不限制）。`max_tokens` 是我们「要求模型最多生成多少」的上限，并非接收端限制；**推理型模型的「思考过程」也占用此额度**，限制过小会导致思考占满额度、答案为空而被截断。关闭后系统不下发 `max_tokens`，由模型按自身上下文上限自由生成；云端按量计费时可开启并设为 `16000` 以上以控制成本 |
| 思考模式 | **本地 / 推理型模型推荐开启「抑制思考」**（默认关闭 = 跟随模型）。开启后：① 对支持的模型（云端 Qwen3 / DeepSeek 等）通过标准参数尽力关闭「思考过程」；② 对**所有**模型启用「思考失控保护」——当模型思考超过「思考上限（字，默认 15000）」仍未开始作答时提前中止，避免推理型模型在整卷上思考数万字、长时间空转。注：部分本地服务（如某些 LM Studio 模型）会忽略关闭思考的参数，此时由「思考失控保护」兜底 |
| 系统提示词 | 留空则使用内置批改提示词，一般无需修改 |

- 添加 / 编辑后可点击「测试连接」，系统会真实拉取该供应商的模型列表并发一条最小对话以验证可用性。
- 在供应商列表中点击「设为当前」即可切换实际使用的供应商。

### 三、批改流程
1. 选择试卷、学生，上传该生答卷照片（可多张）。
2. 系统创建**异步批改任务**并立即返回，后台调用模型；前端每 2.5s 自动轮询进度（无次数上限，慢模型请耐心等待）。开启「流式响应」后，详情弹窗会实时显示「模型思考中/作答中，已生成 N 字（用时 Xs）」，让长耗时批改有明确反馈。
3. 批改完成后展示逐题得分、点评与总分，老师可人工修改。
4. 点击「采纳」将结果写入该生成绩；也可「导出 Excel」留存。

### 四、常见问题
- **提示「未识别接口地址 / base_url 需以 /v1 结尾」**：本地服务多为 `/v1` 路径，系统已自动兜底；若仍失败，请手动将 base_url 补全为以 `/v1` 结尾。
- **提示「模型输出被截断：思考过程占满了 Tokens」**：推理型模型的思考耗尽了「限制 Tokens」额度。**首选在供应商配置中关闭「限制 Tokens」**（不限制输出长度）；若需保留限制，请调大到 `16000` 以上，或减少单次上传的图片数量。
- **批改很慢 / 长时间 processing**：本地推理型模型批改整卷非常慢——「思考 + 作答」实测一张卷约需 6~8 分钟属正常，请耐心等待、不要中途关闭页面（任务在后台异步执行，稍后回来看结果即可）。**开启「流式响应」后会显示实时进度，且改用「空闲超时」守护（默认 300s 无输出才判超时，已覆盖多图首字节等待），长卷不会再被总时长误杀**。相关阈值可用环境变量调整：`AI_GRADING_TIMEOUT_MS`（非流式总超时，默认 15 分钟）、`AI_STREAM_IDLE_TIMEOUT_MS`（流式空闲超时，默认 300s）。
- **模型思考失控 / 长时间 processing 却不出结果**：部分本地推理型模型会在整卷上「思考」数万字仍不作答。**在供应商配置中开启「思考模式 = 抑制思考」**：系统会尽力关闭思考，并对所有模型启用「思考失控保护」——思考超过「思考上限（字，默认 15000）」仍未作答即中止并给出明确提示，不再无限空转。同时建议减少单次上传的图片数量、或更换更快 / 非推理型模型。相关阈值可用环境变量 `AI_THINKING_MODE=suppress`、`AI_REASONING_LIMIT`（思考上限）、`AI_REASONING_RUNAWAY_LIMIT`（default 模式兜底阈值，默认 40000）调整。
- **模型返回的 JSON 无法解析 / 结构损坏**：系统已内置**多级容错**——先直接解析，失败则自动修复常见小错误（如漏逗号、多余尾逗号）再解析，仍失败则「容错打捞」出其中完整的题目与总分（结果会标注「已容错恢复 N 道题，请人工核对」）。若彻底无法恢复，请确认使用的是多模态模型、图片清晰完整、温度较低（`0~0.3`），并可开启「抑制思考」精简输出、关闭「限制 Tokens」避免截断。

### 五、隐私与安全
- 上传的试卷图片以 base64 内联方式发送给所配置的模型服务；请确认该服务（尤其是云端 / 第三方）符合你的数据合规要求。使用本地 / 内网模型可避免图片外发。
- API Key 存储于服务端数据库，前端展示时做掩码处理。
...
- [x] AI 试卷批改（多模态 + 多供应商可切换，异步任务 + 成绩采纳 / 导出）
